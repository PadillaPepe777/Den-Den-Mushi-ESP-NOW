...
float micVolume=0.18;

// Cambia por la MAC del peer (en cada ESP pones la MAC del otro)
uint8_t peerAddress[6] = {0xCC, 0xDB, 0xA7, 0x96, 0xF5, 0xD0};
//2.- {0x3C, 0x8A, 0x1F, 0x5D, 0x7F, 0x1C}
//1.- {0xCC, 0xDB, 0xA7, 0x96, 0xF5, 0xD0}
...
void onDataRecv(const esp_now_recv_info_t* info, const uint8_t* data, int len) {
  ...
  if (CallState == IN_CALL && !isTransmitting && len > 1) {
    ...
    for (size_t i = 0; i < samples; ++i) {
      ...
      audioBuffer[i] = (int16_t)(iir_state * micVolume);
      //Si hay problema de volumen agregar     audioBuffer[i] = (int16_t)(iir_state * micVolume);
      //originalmente era audioBuffer[i] = (int16_t)iir_state;
...
void loop() {
  ...
  if (callBtn == LOW && lastCallBtn == HIGH) {
    ...
    if (CallState == IDLE) {
      ...
      CallState = IN_CALL;
      ringPlayingRemote = false;
      // play answer tone once locally and open eyes
      digitalWrite(Eyes, HIGH);
...
if ((ringPlayingLocal || ringPlayingRemote) && !isTransmitting) {
    playWavStreamChunk(denden_esperando_wav, denden_esperando_wav_len, 0.1f, ringOffset);
}
